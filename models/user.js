...
class User {
  ...
  static async get(username) {
    ...
    // Get list of job IDs the user applied for
    const appsRes = await db.query(
      `SELECT job_id FROM applications WHERE username = $1`,
      [username]
    );
    user.jobs = appsRes.rows.map(r => r.job_id);
  ...
  static async applyToJob(username, jobId) {
    ...
    if (!userCheck.rows.length) throw new NotFoundError(`No user: ${username}`);

    // Check job exists
    const jobCheck = await db.query(
      `SELECT id FROM jobs WHERE id = $1`, [jobId]);
    if (!jobCheck.rows.length) throw new NotFoundError(`No job: ${jobId}`);
