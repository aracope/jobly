...
describe("GET /companies", function () {
  ...
  test("works with multiple filters together", async function () {
    const resp = await request(app).get("/companies?minEmployees=2&maxEmployees=3&nameLike=2");
    expect(resp.body).toEqual({
      companies: [
        {
          handle: "c2",
  ...
  test("responds with 400 for invalid filter key", async function () {
    const resp = await request(app)
      .get("/companies?invalidFilter=xyz")
      .expect(400);
